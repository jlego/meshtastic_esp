...
{
    LOG_INFO("PowerFSM: DARK enter (screen off)");
...
{
    // LOG_INFO("PowerFSM: POWER enter (screen on)");
    if (!isPowered()) {
...
static TimedTransition *onToDarkTT = nullptr;
static TimedTransition *powerToDarkTT = nullptr;

void powerFSM_resetScreenTimer()
{
    unsigned long now = millis();
    if (onToDarkTT && powerFSM.getState() == &stateON)
        onToDarkTT->start = now;
    if (powerToDarkTT && powerFSM.getState() == &statePOWER)
        powerToDarkTT->start = now;
}
...
#endif
    {
        onToDarkTT = powerFSM.add_timed_transition(&stateON, &stateDARK,
                                      Default::getConfiguredOrDefaultMs(config.display.screen_on_secs, default_screen_on_secs),
                                      NULL, "Screen-on timeout");
        powerToDarkTT = powerFSM.add_timed_transition(&statePOWER, &stateDARK,
                                      Default::getConfiguredOrDefaultMs(config.display.screen_on_secs, default_screen_on_secs),
                                      NULL, "Screen-on timeout");
    }
...
#elif defined(MESHTASTIC_EXCLUDE_WIFI)
