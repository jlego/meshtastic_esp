...
int32_t Screen::runOnce()
{
    ...
    {
        int8_t currentFrameIndex = ui->getUiState()->currentFrame;
        bool isOnClockface = (framesetInfo.positions.clock != 255 && currentFrameIndex == framesetInfo.positions.clock);
        if (!isOnClockface || (powerStatus && powerStatus->getHasUSB())) {
            powerFSM_resetScreenTimer();
        }
    }
#endif
