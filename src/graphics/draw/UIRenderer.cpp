...
void UIRenderer::drawGps(OLEDDisplay *display, int16_t x, int16_t y, const meshtastic::GPSStatus *gps)
{
    ...
    if (!gps->getIsConnected()) {
        ...
        const meshtastic_NodeInfoLite *_ourNode = nodeDB->getMeshNode(nodeDB->getNodeNum());
        if (_ourNode && nodeDB->hasValidPosition(_ourNode)) {
            snprintf(textString, sizeof(textString), "0 sats");
        } else
#endif
...
void UIRenderer::drawNavigationBar(OLEDDisplay *display, OLEDDisplayUiState *state)
{
    ...
    for (size_t i = pageStart; i < pageEnd; ++i) {
        ...
        if (isActive) {
#if GRAPHICS_TFT_COLORING_ENABLED
            registerTFTColorRegionDirect(x - 1, y - 1, iconSize + 2, iconSize + 2, TFTPalette::Good, TFTPalette::Black);
            display->setColor(WHITE);
